...
if (isset($update->message)) {
    ...
    $username = isset($update->callback_query->from->username) ? '@' . $update->callback_query->from->username : "не имеет";
}
...
if ($from_id == $config['dev']) {
    if ($test_account_setting['status'] == 'active' and $user['test_account'] == 'no') {
        $start_key = json_encode(['keyboard' => [
            [['text' => '🔧 Управление']],
            [['text' => '🛍 Мои услуги'], ['text' => '🛒 купить услугу']],
            [['text' => '🎁 Тестовый сервис (бесплатно)']],
            [['text' => '👤 профиль'], ['text' => '🛒 Ты знаешь услуги'], ['text' => '💸 пополнить счет']],
...
$select_diposet_payment = json_encode(['inline_keyboard' => [
    [['text' => 'Карта за картой', 'callback_data' => 'kart']],
    [['text' => '▫️Зарин Пал', 'callback_data' => 'zarinpal'], ['text' => 'Идентификатор Р', 'callback_data' => 'idpay']],
...
$panel = json_encode(['keyboard' => [
    ...
    [['text' => '⚙️ Настройки'], ['text' => '👮‍ Административное управление']],
    [['text' => '🔙 возвращается']],
], 'resize_keyboard' => true]);
...
$manage_setting = json_encode(['keyboard' => [
    ...
    [['text' => 'Установка текста робота'], ['text' => 'Настройки платежного шлюза']],
...
$manage_spam = json_encode(['inline_keyboard' => [
    ...
    [['text' => ($spam_setting['type'] == 'ban') ? '🚫 заблокирован' : '⚠️ Предупреждение', 'callback_data' => 'change_type_spam'], ['text' => '▫️Модель столкновения :', 'callback_data' => 'null']],
    [['text' => $spam_setting['time'] . ' Второй', 'callback_data' => 'change_time_spam'], ['text' => '▫️Время : ', 'callback_data' => 'null']],
    [['text' => $spam_setting['count_message'] . 'число', 'callback_data' => 'change_count_spam'], ['text' => 'Количество сообщений : ', 'callback_data' => 'null']],
]]);

$manage_payment = json_encode(['keyboard' => [
    [['text' => '✏️ Статус отключения/включения платежного шлюза бота']],
    [['text' => 'Установка владельца номера карты'], ['text' => 'Установка номера карты']],
    [['text' => '▫️Зарин Пал'], ['text' => 'Идентификатор Р']],
    [['text' => '◽ NOWPayments']],
    [['text' => '⬅️ Вернуться к управлению']]
], 'resize_keyboard' => true]);
...
$manage_off_on_paymanet = json_encode(['inline_keyboard' => [
    ...
    [['text' => ($payment_setting['idpay_status'] == 'active') ? '🟢' : '🔴', 'callback_data' => 'change_status_idpay'], ['text' => '▫️Идентификатор Р :', 'callback_data' => 'null']],
    ...
    [['text' => ($payment_setting['card_status'] == 'active') ? '🟢' : '🔴', 'callback_data' => 'change_status_card'], ['text' => 'Карта за картой :', 'callback_data' => 'null']]
]]);

$manage_texts = json_encode(['keyboard' => [
    [['text' => '✏️ Текст тарифа услуги'], ['text' => '✏️ Начальный текст']],
